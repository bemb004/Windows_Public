...
if ($resolvedType -eq 'tomcat') {
    ...
    # FUNCTION: Read Tomcat log directory from Windows Procrun registry
    ...
    # DETECTION: Determine Tomcat log path for component
    ...
    # DETECTION: Identify installed Tomcat version for this component
    $CurrentTomcatVersion = Get-CurrentTomcatVersion -ComponentName $ComponentName

    # VALIDATION: Ensure Tomcat version could be detected
    ...
    # VALIDATION: Check version format and extract major version
    ...
    # INFO: Final target Tomcat version for patching
    Write-Host "Final target Tomcat version: $NewTomcatVersion" -ForegroundColor Green

    # VALIDATION: Ensure Tomcat log path could be resolved
    if (-not $LogTarget) {
    ...
    # FUNCTION: Verify valid Tomcat home directory (catalina.bat present)
    ...
    # FUNCTION: Locate existing Tomcat artifacts by version in search paths
    ...
    # FUNCTION: Smart extraction for ZIP / TAR / TAR.GZ / TGZ archives
    ...
    # FUNCTION: Ensure Tomcat is ready at target path (search, extract, normalize)
    ...
    # FUNCTION: Update version info inside tomcat.senv for specific component
    ...
    # FUNCTION: Update Windows service registry and procrun parameters
    ...
    # EXECUTION: Prepare Tomcat, update senv and apply service registry changes
    try {
    ...
    if ($service) {
        ...
        # VALIDATION: Compare active Tomcat version with target version
        if ($service.Status -eq 'Running') {
            try {
                $svcRoot = "HKLM:\SYSTEM\CurrentControlSet\Services\$ComponentName"
                $imgPath = (Get-ItemProperty -Path $svcRoot -Name ImagePath -ErrorAction Stop).ImagePath
    ...
    # RESULT: Output final patching / version validation summary
    if ($success) {
    ...
    # FINAL FIX: Force tomcat.senv encoding to UTF-8 (no BOM)
...
function Update-ApacheServiceVersion {
    ...
    if (-not (Test-Path $svcRoot)) {
        throw "Service not found in registry: $svcRoot"
    }

    $newHttpd = Join-Path $ApacheRoot "$NewApacheVersion\bin\httpd.exe"

    if (-not (Test-Path $newHttpd)) {
        throw "New httpd.exe not found: $newHttpd"
    }

    # New ImagePath (Windows Service Startup)
    $imagePath = "`"$newHttpd`" -k runservice"

    # New Description
    $description = "Apache HTTP Server $NewApacheVersion - patched by Internet Services"

    Write-Host "Updating registry for Apache service $ServiceName..." -ForegroundColor Cyan
...
try {
    ...
    if (Test-Path $ApacheSenvPath) {
        Write-Host "Converting apache.senv to UTF-8 (no BOM)..." -ForegroundColor Yellow
